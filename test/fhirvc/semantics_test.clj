(ns fhirvc.semantics-test
  (:require [clojure.test :refer :all]
            [fhirvc.semantics :refer [diff]]
...
(deftest returns-nil-when-there-are-no-structure-changes
  (let [structure-definition-without-changes
        (struct-diff/create {} {} {}
                            {"name" "Account"})]                           
    (is (= (diff structure-definition-without-changes)
           (list)))))
...
(deftest returns-nil-when-there-are-no-semantic-changes
  (let [structure-definition-without-semantic-changes
        (struct-diff/create {"derivation" "constraint"
                            ...
                            {}
                            {"name" "Account"})]                             
    (is (= (diff structure-definition-without-semantic-changes)
           (list)))))
...
(deftest identifies-element-definition-addition
  (let [sd (structure-definition-with [{"path" "Account.owner"
                                       ...
                                       "max" 1}] [] [] [])]                                     
    (is (= (diff sd)
           (list {:priority 1 :name "Account.owner" :text "added"})))))
...
(deftest identifies-element-definition-removal
  (let [sd (structure-definition-with []
                                      ...
                                      [])]                                          
    (is (= (diff sd)
           (list {:priority 1 :name "Account.owner" :text "removed"})))))
...
(deftest identifies-element-definition-rename
  (let [sd (structure-definition-with [{"path" "Account.active"
                                      ...
                                      [])]
    (is (= (diff sd)
           (list {:priority 1 :name "Account.active" :text "Renamed from Account.activePeriod to Account.active"})))))
...
(deftest identifies-type-change-in-element-definition
  (let [sd (structure-definition-with []
                                      ...
                                      [])]
    (is (= (diff sd)
           (list {:priority 1 :name "Account.balance" :text "Type changed from Quantity to Money"})))))
...
(deftest identifies-cardinality-change-in-element-definition 
  (let [sd (structure-definition-with []
                                      ...
                                      [])]                                                                                                         
    (is (= (diff sd)
           (list {:priority 1 :name "Account.status" :text "Max cardinality changed from 1 to *"})))))
...
(deftest identifies-binding-change
  (let [sd (structure-definition-with []
                                      ...
                                      [])]
    (is (= (diff sd)
           (list {:priority 2 :name "Account.status" :text "Binding strength changed from preferred to required"})))))
...
(deftest identifies-short-definition-change
  (let [sd (structure-definition-with []
                                      ...
                                      [])]
    (is (= (diff sd)
           (list {:priority 3 :name "AllergyIntolerance.status" :text "Definition for xml presentation changed from old to new"})))))
